...
const Filters: IFilters = {
  ...
  sortBy(items, field) {
    const sorters = {
      release_date: (itemA: IMovie, itemB: IMovie) => getTime(itemB[field]) - getTime(itemA[field]),
      vote_average: (itemA: IMovie, itemB: IMovie) => (itemB[field] as number) - (itemA[field] as number),
    };
